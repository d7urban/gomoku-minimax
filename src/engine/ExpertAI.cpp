...
constexpr int kExpertDepthCeiling = 24;
...
SearchResult ExpertAI::chooseMove(const GameState& state, Player player, SearchConfig config) {
    ...
    config.maxDepth = std::max(config.maxDepth, kExpertDepthCeiling);
    ...
    config.useReverseFutilityPruning = false;
    config.useRazoring = false;
